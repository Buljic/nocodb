...
// Record locked read-only by an RLS read_only policy — disable every field.
const isRlsReadonly = computed(() => !!(_row.value?.rowMeta?.isRlsReadonly || (_row.value?.row as any)?.__nc_rls_readonly))

const readOnly = computed(() => !isUIAllowed('dataEdit') || isPublic.value || isSqlView.value || isRlsReadonly.value)
...
const addBlueprintForColumn = async (col: ColumnType) => {
  ...
  try {
    ...
    openExpandedFormDetached({
      ...
      breadcrumbs,
      newRecordSubmitBtnText: t('labels.saveRecord'),
      newRecordHeader: t('labels.newRecordForTable', { tableName: relatedMeta.title }),
...
const getRelatedTableName = (col: ColumnType): string => {
  ...
  const relatedTableId = colOptions?.fk_related_model_id
  const baseId = meta.value?.base_id
  if (!relatedTableId || !baseId) return t('objects.subRecord')
  const { getMetaByKey } = useMetas()
  const relatedMeta = getMetaByKey(baseId, relatedTableId)
  return relatedMeta?.title || t('objects.subRecord')
}
...
<template>
  ...
  >
    ...
    >
      ...
      >
        <template #title>{{
          isParentLtarColumn(col) ? $t('tooltip.fieldAutoLinkedToParent') : $t('msg.info.fieldReadonly')
        }}</template>
        <PermissionsTooltip
          v-if="col.title"
          class="w-full"
